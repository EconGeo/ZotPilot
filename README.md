...
| 整理 | `manage_tags`、`manage_collections`、`create_note`、`delete_note` |
| 引用 | `get_citations` |
...
Key highlights:
- **Token-aware chunker:** Install with `pip install -e '.[llamaindex]'` and enable via `chunker_backend: "llamaindex"` to ensure chunks respect embedding model limits.
